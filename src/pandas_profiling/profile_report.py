...
class ProfileReport(SerializeReport, ExpectationsReport):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if explorative:
            config = config.update(Config.get_arg_groups("explorative"))
        if sensitive:
            config = config.update(Config.get_arg_groups("sensitive"))
        if dark_mode:
            config = config.update(Config.get_arg_groups("dark_mode"))
        if orange_mode:
            config = config.update(Config.get_arg_groups("orange_mode"))
        if len(kwargs) > 0:
            config = config.update(Config.shorthands(kwargs))
